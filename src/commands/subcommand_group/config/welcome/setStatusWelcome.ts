...
export default new Command({
	...
	options: [
		{
			name: 'status',
			description: 'Estado de las bienvenidas',
	...
	run: async ({ interaction, client, args }) => {
		...
		try {
			...
			.setTitle('✅ | Estado de bienvenidas establecido')
			.setDescription(`El estado de las bienvenidas ha sido establecido correctamente a \`${status ? 'Activado' : 'Desactivado'}\``)
			.setColor('Green')
			.setTimestamp()
